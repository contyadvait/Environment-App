...
struct WaterUsageView: View {
    ...
    @State var electricalUsage: Double = 0
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                Text("Track your water usage here")
                    .font(.custom("Crimson Pro", size: 24))
                Spacer()
            }
            HStack {
                TextField("Enter Water Usage Here", value: $electricalUsage, format: .number)
        ...
        .customAlert(isPresented: $confirm) {
            VStack {
                ...
                HStack {
                    Text("You have used \(electricalUsage)m³ in the month of \(month.displayName). You may loose credits if you have used too much.")
                        .font(.custom("Josefin Sans", size: 16))
            ...
            Button {
                if electricalUsage <= 12.5 {
                    userData.credits = userData.credits + 10
                } else {
                    userData.credits = userData.credits - 10
                }
                userData.housingData.append(HousingData(month: month, amount: Float(electricalUsage), type: .water))
...
#Preview {
    WaterUsageView(userData: .constant(StorageData(name: "Advait",
